...
package com.dzbkq.bmapcluster.view;
...
import com.dzbkq.bmapcluster.R;
...
import com.dzbkq.bmapcluster.cluster.ClusterRenderer;
import com.dzbkq.bmapcluster.cluster.MarkerManager;
import com.dzbkq.bmapcluster.cluster.Pointd;
import com.dzbkq.bmapcluster.cluster.SphericalMercatorProjection;
import com.dzbkq.bmapcluster.utils.SetFromMapCompat;
...
public class DefaultClusterRenderer<T extends ClusterItem> implements ClusterRenderer<T> {
     ...
     */
    private Set<MarkerWithPosition> mMarkers = newSetFromMapCompat(
    ...
    public void onAdd() {
        ...
        mClusterManager.getClusterMarkerCollection().setOnMarkerClickListener(new OnMarkerClickListener() {
            @Override
            public boolean onMarkerClick(Marker marker) {
                return mClickListener != null && mClickListener.onClusterClick(mMarkerToCluster.get(marker), marker);
            }
        });
    }
    ...
    private class ViewModifier extends Handler {
        ...
        public void handleMessage(Message msg) {
            ...
            try {
            	...
            	renderTask.setVisibleBounds(mMap.getMapStatus().bound);
    ...
    private class RenderTask implements Runnable {
        ...
        public void setVisibleBounds(LatLngBounds bounds) {
        ...
        public void run() {
            ...
            List<Pointd> existingClustersOnScreen = null;
            if (DefaultClusterRenderer.this.mClusters != null && SHOULD_ANIMATE) {
                existingClustersOnScreen = new ArrayList<Pointd>();
                for (Cluster<T> c : DefaultClusterRenderer.this.mClusters) {
                    if (shouldRenderAsCluster(c) && visibleBounds.contains(c.getPosition())) {
                        Pointd point = mSphericalMercatorProjection.toPoint(c.getPosition());
            ...
            final Set<MarkerWithPosition> newMarkers = newSetFromMapCompat(
                    new ConcurrentHashMap<MarkerWithPosition, Boolean>());
            for (Cluster<T> c : clusters) {
                boolean onScreen = visibleBounds.contains(c.getPosition());
                if (zoomingIn && onScreen && SHOULD_ANIMATE) {
                    Pointd point = mSphericalMercatorProjection.toPoint(c.getPosition());
                    Pointd closest = findClosestCluster(existingClustersOnScreen, point);
                    if (closest != null) {
            ...
            // markers to animate from.
            List<Pointd> newClustersOnScreen = null;
            if (SHOULD_ANIMATE) {
                newClustersOnScreen = new ArrayList<Pointd>();
                for (Cluster<T> c : clusters) {
                    if (shouldRenderAsCluster(c) && visibleBounds.contains(c.getPosition())) {
                        Pointd p = mSphericalMercatorProjection.toPoint(c.getPosition());
            ...
            for (final MarkerWithPosition marker : markersToRemove) {
                ...
                if (!zoomingIn && zoomDelta > -3 && onScreen && SHOULD_ANIMATE) {
                    final Pointd point = mSphericalMercatorProjection.toPoint(marker.position);
                    final Pointd closest = findClosestCluster(newClustersOnScreen, point);
                    if (closest != null) {
    ...
    private static Pointd findClosestCluster(List<Pointd> markers, Pointd point) {
        if (markers == null || markers.isEmpty()) return null;

        // TODO: make this configurable.
        double minDistSquared = MAX_DISTANCE_AT_ZOOM * MAX_DISTANCE_AT_ZOOM;
        Pointd closest = null;
        for (Pointd candidate : markers) {
    ...
    @SuppressLint("NewApi")
	public static <E> Set<E> newSetFromMapCompat(Map<E, Boolean> map) {
		if (map.isEmpty()) {
			if (Build.VERSION.SDK_INT <= Build.VERSION_CODES.FROYO) {
                return new SetFromMapCompat<E>(map);
			}
			else {
